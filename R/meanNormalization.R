...
#'        (either zero or \code{NA}). Options are:
#'        \itemize{
#'          \item \code{"warn"} (default): Warn and leave the row unnormalized.
#'          \item \code{"error"}: Stop execution with an error.
#'          \item \code{"omit"}: Remove rows with invalid means from the result.
#'        }
...
#'
#' @return If \code{returnDetails = FALSE}, a normalized matrix with the same dimensions as the input. \cr
#'         If \code{returnDetails = TRUE}, a named list:
#' \itemize{
...
#'
#' # Normalize the matrix and view diagnostics
