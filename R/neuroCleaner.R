...
#' @param demo `data.frame`, a dataframe containing demographic data formatted according to the \link[neuroSCC]{demoCleaner} function. If not provided, only image data will be processed.
#' @return A `data.frame` that combines the NIFTI image data with the demographic data in the appropriate format. Each row represents a pixel, and the columns include demographic data and pixel intensity. If `demo` is not provided, the dataframe will contain only the image data.
...
#' If the demographic dataframe is provided, it checks if it contains the necessary columns and extracts the data for the specified participant.
#' Finally, it combines these data with the image data and returns the resulting dataframe for that patient. If `demo` is not provided, it returns only the image data.
#' @export
#'
...
neuroCleaner <- function(name, demo = NULL) {
  ...
  # If demographic data is provided, merge it with the main dataframe
  if (!is.null(demo)) {
